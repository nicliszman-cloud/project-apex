    ...
    <Screen>
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.profile}>
          {profile?.avatarUrl ? <Image source={{ uri: profile.avatarUrl }} style={styles.avatarImage} /> : <View style={styles.avatar}><Text style={styles.avatarText}>{(profile?.displayName || 'A')[0].toUpperCase()}</Text></View>}
          <View style={{ flex: 1 }}><Text style={styles.name}>{profile?.displayName || 'Seu perfil'}</Text><Text style={styles.handle}>{profile?.username ? '@' + profile.username : '@defina_seu_username'} • {location}</Text></View>
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 64, height: 64, borderRadius: 22, backgroundColor: theme.colors.accent, alignItems: 'center', justifyContent: 'center' },
