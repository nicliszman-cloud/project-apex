...
export default function CarEditScreen(){
  ...
  const [replacementImages,setReplacementImages]=useState<LocalImage[]>([]);const [saving,setSaving]=useState(false);
  ...
  async function save(){
    ...
    try{
      ...
      if(replacementImages.length){
        ...
        const urls:string[]=[];
        for(const image of replacementImages) urls.push(await uploadPublicImage(myUserId,image,'cars/'+car.id));
...
const styles=StyleSheet.create({
  ...
  multiline:{minHeight:105,paddingTop:12},row:{flexDirection:'row',gap:10},chips:{flexDirection:'row',flexWrap:'wrap',gap:8},chip:{borderWidth:1,borderColor:theme.colors.border,borderRadius:18,paddingHorizontal:12,paddingVertical:8,backgroundColor:theme.colors.surface},
